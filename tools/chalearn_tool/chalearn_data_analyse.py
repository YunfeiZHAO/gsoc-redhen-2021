...
class ChalearnAnnotation:
    ...
    def fromText(self, root, type, text):
        ...
        # set paths
        self.video_path = os.path.join(root, type, self.id + '.M.avi')
        self.frame_path = os.path.join(root, 'frames', type, self.id)
        self.keypoint_path = os.path.join(root, 'keypoints', type, self.id + '.json')
        # video properties
        frame_count, fps, width, height = video2frame.get_video_property(self.video_path)
